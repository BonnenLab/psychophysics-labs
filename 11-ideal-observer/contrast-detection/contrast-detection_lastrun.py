...
expName = 'contrast-detection'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/kbonnen/Documents/work-repos/psychophysics-labs/11-ideal-observer/contrast-detection/contrast-detection_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    grating = visual.GratingStim(
        ...
        ori=0.0, pos=(0, 0), draggable=False, size=(0.5, 0.5), sf=4.0, phase=0.0,
    ...
    noise = visual.NoiseStim(
        ...
        phase=0.0,
        color=[1,1,1], colorSpace='rgb',     opacity=None, blendmode='add', contrast=0.2,
        texRes=128, filter=None,
        noiseType='Normal', noiseElementSize=[0.00625], 
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('contrast-detection-2ifc.csv'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[grating, grating_2, noise, noise_2, key_resp],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        grating.setContrast(contrast_1)
        grating_2.setContrast(contrast_2)
        ...
        while continueRoutine:
            ...
            if grating_2.status == NOT_STARTED and tThisFlip >= 0.6-frameTolerance:
                # keep track of start time/frame for later
            ...
            if noise_2.status == NOT_STARTED and tThisFlip >= .6-frameTolerance:
                # keep track of start time/frame for later
            ...
            if noise_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > noise_2.tStartRefresh + .5-frameTolerance:
                    # keep track of stop time/frame for later
