...
# -*- coding: utf-8 -*-
"""
3D Motion Direction Estimation  —  Lab 10
...
# Response indicator: a circle lying in the xz plane (bird's-eye disk),
# perspective-projected to screen so it appears as a foreshortened ellipse.
INDICATOR_Y_CM = 6.0   # cm below eye level (sets how far down on screen)
INDICATOR_R_CM = 8.0   # circle radius in world space (cm)
...
# Response indicator — a ring + radial line + dot at each end.
# The circle lies in the xz plane (bird's-eye view), perspective-projected to screen.
# It appears as a foreshortened ellipse below fixation.
#
# Projection: point (x, −Y, D+z_off) → screen deg (arctan(x/z), arctan(−Y/z))
# Convention: angle=0 → RIGHT (+x), angle=90° → AWAY (+z, into screen)

def _ind_project(x_cm, z_offset_cm):
    """Project an xz-plane indicator point to screen degrees."""
    z = VIEWING_DISTANCE_CM + z_offset_cm
    return (np.degrees(np.arctan2(x_cm,           z)),
            np.degrees(np.arctan2(-INDICATOR_Y_CM, z)))

# Precompute circle vertices (128-sided polygon in perspective)
_ind_t       = np.linspace(0, 2 * np.pi, 128, endpoint=False)
_ind_verts   = np.array([
    _ind_project(INDICATOR_R_CM * np.cos(t), INDICATOR_R_CM * np.sin(t))
    for t in _ind_t
])
_ind_center  = _ind_project(0.0, 0.0)   # projected centre of the disk

indicator_circle = visual.ShapeStim(
    win=win, vertices=_ind_verts,
    lineColor=COL_WHITE, fillColor=None, colorSpace='rgb',
    lineWidth=1.5, closeShape=True,
)
# Line from centre to tip; start/end updated each frame.
indicator_line = visual.Line(
    win=win, start=_ind_center, end=_ind_center,
    lineColor=COL_WHITE, colorSpace='rgb', lineWidth=2,
)
...
# Small dot at the outer tip — position updated each frame
...
# Cardinal labels — positioned just outside the projected circle at each cardinal direction
def _lbl_pos(angle_deg, r_factor=1.4):
    rad = np.deg2rad(angle_deg)
    r   = INDICATOR_R_CM * r_factor
    return _ind_project(r * np.cos(rad), r * np.sin(rad))

_lbl = dict(win=win, height=0.45, color=COL_DIM, colorSpace='rgb')
lbl_right  = visual.TextStim(**_lbl, text='RIGHT',  pos=_lbl_pos(0))
lbl_away   = visual.TextStim(**_lbl, text='AWAY',   pos=_lbl_pos(90))
lbl_left   = visual.TextStim(**_lbl, text='LEFT',   pos=_lbl_pos(180))
lbl_toward = visual.TextStim(**_lbl, text='TOWARD', pos=_lbl_pos(270))
...
for trial_num, trial in enumerate(trial_list):
    ...
    while not responded:
        ...
        if not responded:
            ...
            # Project tip position: angle=0 → RIGHT (+x), angle=90° → AWAY (+z)
            rad = np.deg2rad(angle)
            tip = _ind_project(INDICATOR_R_CM * np.cos(rad),
                               INDICATOR_R_CM * np.sin(rad))
            indicator_line.end   = tip
            indicator_dot_tip.pos = tip
