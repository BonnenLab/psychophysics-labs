...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/kbonnen/Documents/work-repos/psychophysics-labs/09-continuous-psychophysics/tracking/Bonnen2015_tracking_lastrun.py',
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if start_trial_mouse.status == STARTED:  # only update if started and not finished!
                ...
                start_trial_mouse.x.append(float(x))
                start_trial_mouse.y.append(float(y))
        ...
        trial = data.Routine(
            name='trial',
            components=[response_mouse, blob_0, blob, cursor],
        )
        ...
        while continueRoutine and routineTimer.getTime() < 15.5:
            ...
            if response_mouse.status == STARTED:  # only update if started and not finished!
                ...
                response_mouse.x.append(float(x))
                response_mouse.y.append(float(y))
            ...
            if response_mouse.status == STARTED:
                x,y = blob.pos
                blob.x.append(float(x))
                blob.y.append(float(y))
