...
expName = '2d-motion-direction-discrimination'  # from the Builder filename that created this script
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    dots = visual.DotStim(
        ...
        depth=-1.0)
    dots_test = visual.DotStim(
        win=win, name='dots_test',
        nDots=50, dotSize=2.0,
        speed=0.002, dir=1.0, coherence=1.0,
        fieldPos=(0.0, 0.0), fieldSize=0.2, fieldAnchor='center', fieldShape='circle',
        signalDots='same', noiseDots='direction',dotLife=100.0,
        color=[1.0,1.0,1.0], colorSpace='rgb', opacity=None,
        depth=-2.0)
    ...
    for level, condition in staircase:
        ...
        trial = data.Routine(
            name='trial',
            components=[dots, dots_test, key_resp, text_2],
        )
        ...
        while continueRoutine:
            ...
            # if dots_test is starting this frame...
            if dots_test.status == NOT_STARTED and tThisFlip >= .8-frameTolerance:
                # keep track of start time/frame for later
                dots_test.frameNStart = frameN  # exact frame index
                dots_test.tStart = t  # local t and not account for scr refresh
                dots_test.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(dots_test, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'dots_test.started')
                # update status
                dots_test.status = STARTED
                dots_test.setAutoDraw(True)
            
            # if dots_test is active this frame...
            if dots_test.status == STARTED:
                # update params
                pass
            
            # if dots_test is stopping this frame...
            if dots_test.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > dots_test.tStartRefresh + .5-frameTolerance:
                    # keep track of stop time/frame for later
                    dots_test.tStop = t  # not accounting for scr refresh
                    dots_test.tStopRefresh = tThisFlipGlobal  # on global time
                    dots_test.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'dots_test.stopped')
                    # update status
                    dots_test.status = FINISHED
                    dots_test.setAutoDraw(False)
            ...
            if text_2.status == NOT_STARTED and tThisFlip >= 1.3-frameTolerance:
                # keep track of start time/frame for later
        ...
        # update staircase
        if key_resp.keys == 'left' and level!=0:
            staircase.addResponse(1,level)
        elif key_resp.keys == 'right' and level==0:
            staircase.addResponse(1,level)
        else:
