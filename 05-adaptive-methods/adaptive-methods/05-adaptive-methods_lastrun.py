...
expName = '05-adaptive-methods'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/kbonnen/Library/CloudStorage/OneDrive-IndianaUniversity/Courses/V768-Measuring-Perception/V768-Psychophysics-labs/05-adaptive-methods/adaptive-methods/05-adaptive-methods_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_start') is None:
        # initialise key_start
        key_start = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_start',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    key_start = keyboard.Keyboard(deviceName='key_start')
    text = visual.TextStim(win=win, name='text',
        text='This is a two interval forced choice task (2IFC).  On each trial you will see two gratings.  The first will be perfectly vertical. The second will be tilted clockwise (right) or counter-clockwise (left).  Your job is to use the arrow buttons (left and right) to indicate which direction the second grating is tilted relative to the first.\n\nPress the space bar to start.',
        font='Arial',
    ...
    for level, condition in staircase:
        ...
        while continueRoutine:
            ...
            # *grating_test* updates
            
            # if grating_test is starting this frame...
            if grating_test.status == NOT_STARTED and tThisFlip >= 0.6-frameTolerance:
                # keep track of start time/frame for later
                grating_test.frameNStart = frameN  # exact frame index
                grating_test.tStart = t  # local t and not account for scr refresh
                grating_test.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(grating_test, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'grating_test.started')
                # update status
                grating_test.status = STARTED
                grating_test.setAutoDraw(True)
            
            # if grating_test is active this frame...
            if grating_test.status == STARTED:
                # update params
                pass
            
            # if grating_test is stopping this frame...
            if grating_test.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > grating_test.tStartRefresh + .5-frameTolerance:
                    # keep track of stop time/frame for later
                    grating_test.tStop = t  # not accounting for scr refresh
                    grating_test.tStopRefresh = tThisFlipGlobal  # on global time
                    grating_test.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'grating_test.stopped')
                    # update status
                    grating_test.status = FINISHED
                    grating_test.setAutoDraw(False)
        ...
        if key_resp.keys == 'right':
            thisExp.addData('response','cw')
        else:
            thisExp.addData('response','ccw')
