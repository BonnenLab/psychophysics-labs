...
let expName = 'method-of-limits';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'method-of-limits-conditions.csv', 'path': 'method-of-limits-conditions.csv'},
  ]
});
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials = new TrialHandler({
      ...
      trialList: 'method-of-limits-conditions.csv',
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    if (grating.status === PsychoJS.Status.STARTED){ // only update if being drawn
      grating.setContrast(np.exp(-5+direction*(5-t)), false);
    }
