...
expName = 'method-of-adjustment'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/kbonnen/Library/CloudStorage/OneDrive-IndianaUniversity/Courses/V768 - Measuring Perception/demos/week-1/method-of-adjustment/method-of-adjustment_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='This experiment will use the method of adjustment.  \n\nOn each trial you will fixate on the center mark.  There will be two stimuli.  An arrow will point to the the stimulus you can control with the up and down arrows.  Adjust the contrast of that stimulus to match the target stimulus.  Press enter to proceed to the next trial.\n\n\nPress the space bar to begin.',
        font='Arial',
    ...
    adjust = event.Mouse(win=win)
    x, y = [None, None]
    adjust.mouseClock = core.Clock()
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('method-of-adjustment.csv'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[grating, adjust, grating_2, polygon, finish],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        grating.setPos((-control*.15, 0))
        grating.setPhase(360*np.random.rand())
        # setup some python lists for storing info about the adjust
        gotValidClick = False  # until a click is received
        # Run 'Begin Routine' code from update_control
        adjust.setPos((0,-.5+.5*direction))
        grating_2.setPos((control*.15, 0))
        grating_2.setPhase(360*np.random.rand())
        polygon.setOri(control*90)
        ...
        while continueRoutine:
            ...
            if grating.status == NOT_STARTED and tThisFlip >= .250-frameTolerance:
                # keep track of start time/frame for later
            ...
            if grating.status == STARTED:
                # update params
                grating.setContrast(np.exp(-1), log=False)
            # *adjust* updates
            
            # if adjust is starting this frame...
            if adjust.status == NOT_STARTED and t >= 0.25-frameTolerance:
                # keep track of start time/frame for later
                ...
                adjust.mouseClock.reset()
                prevButtonState = [0, 0, 0]  # if now button is down we will treat as 'new' click
            # Run 'Each Frame' code from update_control
            currPos = adjust.getPos()
            
            # *grating_2* updates
            
            # if grating_2 is starting this frame...
            if grating_2.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            if grating_2.status == STARTED:
                # update params
                grating_2.setContrast(np.exp(5*currPos[1]), log=False)
            ...
            if finish.status == NOT_STARTED and tThisFlip >= 0.25-frameTolerance:
                # keep track of start time/frame for later
            ...
            if finish.status == STARTED and not waitOnFlip:
                theseKeys = finish.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
        ...
        # Run 'End Routine' code from update_control
        thisExp.addData('luminance_response',np.exp(5*currPos[1]))
        thisExp.addData('luminance_target',np.exp(-1))
