...
expName = 'method_of_constant_stimuli'  # from the Builder filename that created this script
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('start_exp_resp') is None:
        # initialise start_exp_resp
        start_exp_resp = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='start_exp_resp',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        ...
        languageStyle='LTR',
        depth=0.0);
    start_exp_resp = keyboard.Keyboard(deviceName='start_exp_resp')
    
    # --- Initialize components for Routine "trial" ---
    grating = visual.GratingStim(
        win=win, name='grating',
        tex='sin', mask='raisedCos', anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.25, 0.25), sf=1.0, phase=1.0,
        color=[1,1,1], colorSpace='rgb',
        opacity=None, contrast=1.0, blendmode='avg',
        texRes=512.0, interpolate=True, depth=0.0)
    stimulus_window = visual.ShapeStim(
        win=win, name='stimulus_window',
    ...
    begin_experiment = data.Routine(
        ...
        components=[text, start_exp_resp],
    )
    begin_experiment.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for start_exp_resp
    start_exp_resp.keys = []
    start_exp_resp.rt = []
    ...
    while continueRoutine:
        ...
        # if start_exp_resp is starting this frame...
        ...
        if start_exp_resp.status == STARTED and not waitOnFlip:
            theseKeys = start_exp_resp.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_start_exp_resp_allKeys):
                ...
                start_exp_resp.duration = _start_exp_resp_allKeys[-1].duration
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('yes_no_conditions.csv'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if grating.status == NOT_STARTED and tThisFlip >= np.random.rand()*.15+.1-frameTolerance:
                # keep track of start time/frame for later
            ...
            # *stimulus_window* updates
            
            # if stimulus_window is starting this frame...
            if stimulus_window.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                stimulus_window.frameNStart = frameN  # exact frame index
                stimulus_window.tStart = t  # local t and not account for scr refresh
                stimulus_window.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(stimulus_window, 'tStartRefresh')  # time at next scr refresh
                # update status
                stimulus_window.status = STARTED
                stimulus_window.setAutoDraw(True)
            
            # if stimulus_window is active this frame...
            if stimulus_window.status == STARTED:
                # update params
                pass
            ...
            if response.status == NOT_STARTED and tThisFlip >= 0.75-frameTolerance:
                # keep track of start time/frame for later
            ...
            if polygon.status == NOT_STARTED and tThisFlip >= 0.75-frameTolerance:
                # keep track of start time/frame for later
